...
int DJSession::load_track_to_controller(const std::string& track_name) {
    ...
    if(!track){
        ...
        int loaded = controller_service.loadTrackToCache(*track); //loading the track to cache and checking what happened
        if(loaded==1){ //HIT
            stats.cache_hits++;
        }
        else{
            if(loaded==0){ //MISS (no eviction)
                stats.cache_misses++;
            }
...
bool DJSession::load_track_to_mixer_deck(const std::string& track_title) {
    ...
    // your implementation here
    AudioTrack* track = controller_service.getTrackFromCache(track_title); //getting the track
    if(!track){
        std::cout <<"[ERROR] Track: " << track_title << " not found in cache\n";
        stats.errors++;
        return false;
    }
    int loaded = mixing_service.loadTrackToDeck(*track); //trying to load the track to deck
    if(loaded==0){ //track was loaded to deck A
        stats.deck_loads_a++;
        stats.transitions++;
    }
    else{
        if(loaded==1){ //track was loaded to deck B
            stats.deck_loads_b++;
            stats.transitions++;
        }
        else{ //track failed to load
...
void DJSession::simulate_dj_performance() {
    ...
    if(play_all){
        for(const std::pair<const std::string, std::vector<int>>& pair: session_config.playlists){
            playlists.push_back(pair.first);
        }
        std::sort(playlists.begin(),playlists.end()); //sorting the playlist names by lexicographical order

    }
    while(true){
        //FINDING THE NEXT PLAYLIST TO PLAY
        std::string playlist_name;
        if(play_all){ //we need the next playlist from all playlists in the library
            if(playlist_index>=playlists.size()){ //checking that we didn't get to the end
                break;
            }
            playlist_name = playlists[playlist_index];//getting the next playlist from the library
            playlist_index++;//updating the index for the next playlist
        }
        else{ //we need to ask the user for a playlist
            playlist_name=display_playlist_menu_from_config();
            if(playlist_name.empty()){ //checking that the user still wants a playlist
                break;
            }
        }
        //LOADING PLAYLIST
        bool loaded_playlist = load_playlist(playlist_name);
        if(!loaded_playlist){
            std::cout << "[ERROR] Failed to load playlist '" << playlist_name << "'\n";
            continue; //continuing to the next playlist
        }
        for(const std::string& track_title : track_titles){
            std::cout << "\n--- Processing: " << track_title << " ---\n";
            stats.tracks_processed++;
            load_track_to_controller(track_title); //loading the track to the controller and updating cache statistics
            controller_service.displayCacheStatus();
            if(!load_track_to_mixer_deck(track_title)){ //loading the track to the mixer and updating cache statistics
                continue; //continuing to next track
            }
            mixing_service.displayDeckStatus();
        }
        //ALL TRACKS HAVE BEEN PROCESSED
        print_session_summary();
        /**SETTING ALL STATS MEMBERS TO 0
